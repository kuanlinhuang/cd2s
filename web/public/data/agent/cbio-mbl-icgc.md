...
- Overall Survival (Months): 82.4% populated (one-to-many)
- Progress Free Survival (Months): 79.2% populated (one-to-many)
...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=mbl_icgc
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/mbl_icgc
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/mbl_icgc/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-mbl-icgc.json
