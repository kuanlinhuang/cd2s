...
- Median follow-up: 96.5 months (derivable for 48 cases)
...
- Primary Tumor Site: 50.4% informative
- Overall Survival Status: 49.6% informative
- Sex: 49.6% informative
- Overall Survival (Months): 40.3% populated (one-to-many)
...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=prad_mich
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/prad_mich
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/prad_mich/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-prad-mich.json
