...
- Measurements: Mutation calls (panel, exome or genome), Copy number alterations, RNA sequencing expression, Structured treatment records
- Median follow-up: 32.0 months (derivable for 313 cases)
...
- Overall Survival: 95.4% informative
- Sex: 95.4% informative
- Tumor Grade: 95.4% informative
- Overall Survival (months): 95.1% populated (one-to-many)
- Named therapeutic agents: 78.1% populated (one-to-many)
...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=difg_glass
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/difg_glass
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/difg_glass/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-difg-glass.json
