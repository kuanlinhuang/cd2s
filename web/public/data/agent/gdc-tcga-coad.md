...
- AJCC pathologic stage: 97.2% informative
- Disease response at follow-up: 93.7% populated (one-to-many)
- Treatment type: 88.5% populated (one-to-many)
- Treatment given: 88.5% populated (one-to-many)
- Country of residence: 81.3% informative
- Race: 61.8% informative
- Ethnicity: 59.7% informative
- Therapeutic agents: 34.9% populated (one-to-many)
- Treatment outcome: 20.0% informative
...
2. Download the open files without an account (minutes to hours, by cohort size)
   12,424 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   16,818 controlled-access files hold sequence-level data and are released only under an approved data access request for phs000178. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["TCGA-COAD"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-tcga-coad.json
