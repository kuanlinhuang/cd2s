...
- Prior malignancy: 99.7% informative
- Synchronous malignancy: 99.7% informative
- Disease response at follow-up: 99.7% populated (one-to-many)
- Tumor grade: 98.7% informative
- Country of residence: 97.9% informative
- Race: 97.3% informative
- Ethnicity: 95.0% informative
- AJCC pathologic stage: 93.9% informative
- ECOG performance status: 77.7% informative
...
2. Download the open files without an account (minutes to hours, by cohort size)
   9,302 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   14,781 controlled-access files hold sequence-level data and are released only under an approved data access request for phs000178. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["TCGA-LIHC"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-tcga-lihc.json
