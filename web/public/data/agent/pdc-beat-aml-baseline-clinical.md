...
1. Open the study in the PDC portal [human] (5 minutes)
   ...
   Policy evidence: https://pdc.cancer.gov/pdc/study/PDC000478
...
3. Resolve the study UUID, then fetch the file manifest [agent]
   filesPerStudy keys on the study UUID, not on PDC000478: called with the pdc_study_id it returns the right number of rows with every column null. Resolve the UUID first and pass the terms acknowledgement.
...
- Reuse gap index: -0.94 (negative means less reused than comparable datasets)
  - High SEC61A1 expression predicts poor outcome of acute myeloid leukemia. (2024) PMID 38584833
