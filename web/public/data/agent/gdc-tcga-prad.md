...
- Disease response at follow-up: 98.6% populated (one-to-many)
- Treatment type: 97.8% populated (one-to-many)
- Treatment given: 97.8% populated (one-to-many)
- Prior malignancy: 97.6% informative
- Synchronous malignancy: 97.6% informative
- Race: 97.2% informative
- Ethnicity: 79.0% informative
- Treatment outcome: 23.4% populated (one-to-many)
- Progression at follow-up: 22.6% informative
- Therapeutic agents: 14.8% populated (one-to-many)
...
2. Download the open files without an account (minutes to hours, by cohort size)
   12,425 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   19,793 controlled-access files hold sequence-level data and are released only under an approved data access request for phs000178. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["TCGA-PRAD"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-tcga-prad.json
