...
- Median follow-up: 9.1 months (derivable for 135 cases)
...
- Ethnicity Category: 98.9% informative
- Stage: 86.3% informative
- Progress Free Survival (Months): 73.8% populated (one-to-many)
...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=luad_broad
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/luad_broad
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/luad_broad/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-luad-broad.json
