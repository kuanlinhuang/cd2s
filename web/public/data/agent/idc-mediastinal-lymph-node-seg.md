...
Dataset id: idc-mediastinal-lymph-node-seg
...
- Cohort: 513 cases
- Cancer types: Breast Cancer, Non-small Cell Lung Cancer, Hodgkin Lymphoma, Small Cell Lung Cancer, Thyroid Cancer, Adenocarcinoma
- Subject: pan cancer
...
1. Open the collection in the IDC portal (5 minutes)
   ...
   https://portal.imaging.datacommons.cancer.gov/explore/?collection_id=mediastinal_lymph_node_seg
...
3. Download with idc-index (hours for a full collection)
   ...
   python -c "from idc_index import IDCClient; IDCClient().download_from_selection(collection_id='mediastinal_lymph_node_seg', downloadDir='.')"
   ```
...
4. List the series before pulling any pixels
   ...
   ```
   python -c "from idc_index import IDCClient; df = IDCClient().get_series(collection_id='mediastinal_lymph_node_seg'); print(len(df), df['Modality'].value_counts().to_dict())"
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/idc-mediastinal-lymph-node-seg.json
