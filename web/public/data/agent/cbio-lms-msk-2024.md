...
- Median follow-up: 45.0 months (derivable for 433 cases)
...
- Disease-specific Survival status: 100.0% informative
...
- Progression Free Survival (Months): 100.0% populated (one-to-many)
- Primary Tumor Site: 97.7% informative
- Race: 96.5% informative
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/lms_msk_2024
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/lms_msk_2024/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-lms-msk-2024.json
