...
4. Give the agent a Synapse token, not a password
   ...
   python -c "import synapseclient; syn = synapseclient.login(); print(syn.get('syn39262957', downloadFile=False).name)"
   ```

## Evidence of prior reuse

- Articles that analyzed these data: 0
- Citations to the dataset's publication: 397 (attention, not reuse)
- Reuse gap index: -2.63 (negative means less reused than comparable datasets)
