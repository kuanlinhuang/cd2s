...
- Vital status: 93.9% informative
- Prior treatment: 93.9% populated (one-to-many)
- Prior malignancy: 93.9% informative
- AJCC pathologic stage: 92.4% informative
- AJCC clinical stage: 90.5% informative
- Disease response at follow-up: 90.1% informative
- Race: 88.6% informative
- Ethnicity: 83.3% informative
- ECOG performance status: 75.3% informative
...
1. Open the project in the GDC Data Portal (5 minutes)
   ...
   https://portal.gdc.cancer.gov/projects/TCGA-TGCT
2. Download the open files without an account (minutes to hours, by cohort size)
   5,094 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   7,916 controlled-access files hold sequence-level data and are released only under an approved data access request for phs000178. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["TCGA-TGCT"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Reuse gap index: +0.09 (negative means less reused than comparable datasets)
  ...
  - PUREE: accurate pan-cancer tumor purity estimation from gene expression data. (2023) PMID 37041233
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-tcga-tgct.json
