...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=prad_mskcc
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/prad_mskcc
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/prad_mskcc/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-prad-mskcc.json
