...
- Sex at birth: 97.9% informative
- Vital status: 97.9% informative
...
- Prior malignancy: 96.6% informative
- Synchronous malignancy: 96.6% informative
- Disease response at follow-up: 95.0% populated (one-to-many)
- Race: 92.1% informative
- Ethnicity: 70.0% informative
- Country of residence: 56.2% informative
- Treatment outcome: 22.0% informative
...
2. Download the open files without an account (minutes to hours, by cohort size)
   14,195 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   19,469 controlled-access files hold sequence-level data and are released only under an approved data access request for phs000178. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["TCGA-UCEC"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-tcga-ucec.json
