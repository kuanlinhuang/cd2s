# CPTAC-KF-CCDI-Pediatric T-ALL Study

Dataset id: pdc-cptac-kf-ccdi-pediatric-t-all-study
...
- Tissue or organ of origin: 93.9% informative
- Ethnicity: 92.1% informative
- Age at diagnosis: 91.2% populated (one-to-many)
- Race: 85.1% informative
- Cause of death: 12.3% informative
...
1. Open the study in the PDC portal (5 minutes)
   Proteomic data are released open: no account, no request, no waiting. This cohort spans 2 PDC studies (PDC000641, PDC000640); repeat for each.
   https://pdc.cancer.gov/pdc/study/PDC000641
...
3. Resolve the study UUID, then fetch the file manifest
   filesPerStudy keys on the version UUID, not on PDC000641. Passing the PDC study id returns one row per file with every field null - a successful call that looks like a study whose files carry no metadata. Resolve the latest version through studyCatalog first. acceptDUA: true is the programmatic form of the portal's terms click; without it there are no signed download URLs.
...
- Full structured record: https://cd2s.vercel.app/data/datasets/pdc-cptac-kf-ccdi-pediatric-t-all-study.json
