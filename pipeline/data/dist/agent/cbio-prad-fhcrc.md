...
1. Open the study in cBioPortal (5 minutes)
   ...
   https://www.cbioportal.org/study/summary?id=prad_fhcrc
...
3. Or pull the staging files with git-lfs
   ...
   git -c lfs.fetchexclude="" lfs pull -I public/prad_fhcrc
   ```
...
4. Read the study straight off the REST API
   ...
   ```
   curl -s 'https://www.cbioportal.org/api/studies/prad_fhcrc/clinical-data?clinicalDataType=PATIENT&projection=SUMMARY'
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/cbio-prad-fhcrc.json
