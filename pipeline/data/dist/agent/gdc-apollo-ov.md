# APOLLO2: Proteogenomic characterization of ovarian serous cystadenocarcinoma

Dataset id: gdc-apollo-ov
...
1. Open the project in the GDC Data Portal (5 minutes)
   ...
   https://portal.gdc.cancer.gov/projects/APOLLO-OV
2. Download the open files without an account (minutes to hours, by cohort size)
   260 open-access files are open access: no login, no request. Add them to the portal cart to get a manifest, then fetch it with the GDC Data Transfer Tool.
...
3. Request controlled access through dbGaP (days to a few weeks after signing official approval)
   1,859 controlled-access files hold sequence-level data and are released only under an approved data access request for phs003488. The request starts at dbGaP, not in the portal, and it is your institution's signing official who submits it.
   Requires: eRA Commons account, institutional signing official approval, dbGaP data access request for phs003488
...
4. Download controlled files with your token
   ...
   https://www.ncbi.nlm.nih.gov/projects/gap/cgi-bin/study.cgi?study_id=phs003488
...
5. Query the project from code, with no credentials
   ...
   curl -s 'https://api.gdc.cancer.gov/files' --get \
     --data-urlencode 'filters={"op":"and","content":[{"op":"in","content":{"field":"cases.project.project_id","value":["APOLLO-OV"]}},{"op":"in","content":{"field":"access","value":["open"]}}]}' \
...
- Full structured record: https://cd2s.vercel.app/data/datasets/gdc-apollo-ov.json
