...
- Cohort: 212 cases
- Cancer types: Squamous Cell Carcinoma
- Subject: Lung
...
1. Open the collection in the IDC portal (5 minutes)
   ...
   https://portal.imaging.datacommons.cancer.gov/explore/?collection_id=cptac_lscc
...
3. Download with idc-index (hours for a full collection)
   ...
   python -c "from idc_index import IDCClient; IDCClient().download_from_selection(collection_id='cptac_lscc', downloadDir='.')"
   ```
...
4. List the series before pulling any pixels
   ...
   ```
   python -c "from idc_index import IDCClient; df = IDCClient().get_series(collection_id='cptac_lscc'); print(len(df), df['Modality'].value_counts().to_dict())"
   ```
...
- Full structured record: https://cd2s.vercel.app/data/datasets/idc-cptac-lscc.json
